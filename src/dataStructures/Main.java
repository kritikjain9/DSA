...
public class Main {
  ...
  public static class LinkedList {
    ...
    public static int _findIntersection(LinkedList one, LinkedList two){
    	    	
    	int diff = Math.abs(one.size - two.size);
    	if(one.size > two.size) {
    		return findInter(one, two, diff);
    	}
    	return findInter(two, one, diff);
    }
    
    public static int findInter(LinkedList one, LinkedList two, int diff) {
    	Node p1, p2;
    	p1 = one.head;
    	p2 = two.head;
